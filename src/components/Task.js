...
import {isToday} from "../helpers/dueDate";
...
const Task=(props)=> {
	...
	return (
		...
		>
           ...
           {!isEditting && isDone && (new Date(props.task.dueDate).getDate()<new Date().getDate())  && (
				<div className="flex-none flex flex-row text-xs text-green-500 mr-1 py-1 px-2">
					<CalendarIcon className="h-4 w-4 mr-1" />	 
				<div className="text-green-1000">Completed</div>
				</div>
			)}
			{!isEditting && !isDone && (new Date(props.task.dueDate).getDate()<new Date().getDate())  && (
				<div className="flex-none flex flex-row text-xs text-blue-500 mr-1 py-1 px-2">
					<CalendarIcon className="h-4 w-4 mr-1" />	 
				{dueDate(props.task.dueDate)}-<div className="text-blue-1000"> Over-due</div>
				</div>
			)}
	        {!isEditting && isDone && isToday(props.task.dueDate) && (
				<div className="flex-none flex flex-row text-xs text-green-500 mr-1 py-1 px-2">
					<CalendarIcon className="h-4 w-4 mr-1" />
					<div className="text-green-1000">Completed</div>
				</div>
			...
			{!isEditting && isDone && !isToday(props.task.dueDate)&& (new Date(props.task.dueDate).getDate()>new Date().getDate())
			&& (
				<div className="flex-none flex flex-row text-xs text-green-500 mr-1 py-1 px-2">
					<CalendarIcon className="h-4 w-4 mr-1" />
					<div className="text-green-1000">Completed</div>
				...
				{!isEditting && !isDone && !isToday(props.task.dueDate)&& (new Date(props.task.dueDate).getDate()>new Date().getDate())
			&& (
				<div className="flex-none flex flex-row text-xs text-yellow-500 mr-1 py-1 px-2">
					<CalendarIcon className="h-4 w-4 mr-1" />
					{dueDate(props.task.dueDate)}
				</div>
			)}
